...
- **No retroactive catch-up.** Vertex orders events a node observes *while
  ...
  state-transfer path (`vertex.joining_running_session`, out of scope). N4 keeps the churned robot `active` across the decisions it acts on;
...
| This plan | Existing criterion (design §7) | Relationship |
|---|---|---|
